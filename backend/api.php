...
try {
   ...
   function logmessage($cronname, $status, $message, $query){
	   global $pdo;
	   $insert=$pdo->prepare("INSERT INTO backorder_logs(cron, date, status, message, query) VALUES(:cron, :date, :status, :message, :query)");
	   $insert->execute(array(':cron' => $cronname, ':date' => date("Y-m-d H:i:s") , ':status' => $status, ':message' => $message, ':query' => $query));
   }
   ...
   function backorder_api_check_valid_tld($domain, $userid) {
       ...
       $tlds = "";
       $result=$pdo->prepare("SELECT extension FROM backorder_pricing WHERE currency_id=?");
